...
func (client *Client) GetPost(id int32) (api.Post, error) {
	...
	url := client.Host + "/api/post/" + strconv.FormatInt(int64(id), 10)
	r, err := makeRequest("GET", url, nil)
	if err != nil {
		return resp, err
	}
...
func (client *Client) UpdatePost(post api.Post) (api.Post, error) {
	...
	url := client.Host + "/api/post/" + strconv.FormatInt(int64(post.Id), 10)
